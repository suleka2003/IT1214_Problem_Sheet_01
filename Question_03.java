...
public class Question_03 {
    public static void main(String[] args) {
        Bank bank = new Bank(5); // Initialise bank with capacity for 5 accounts as given in the paper
        ...
        try {
            bank.withdraw(1001, 6000); // Attempting an overdraft
        } catch (IllegalArgumentException e) {
            System.out.println("Error: " + e.getMessage());
        }

        try {
            bank.withdraw(1002, 1000); // Successful withdrawal
        } catch (IllegalArgumentException e) {
            System.out.println("Error: " + e.getMessage());
        }

        bank.displayAccounts(); // Display all accounts
    }
}